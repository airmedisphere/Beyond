...
from utils.clients import initialize_clients  # Imported but called lazily
...
async def get_clients():
    """Lazy initialization of Telegram clients to avoid OOM at startup."""
    global _clients
    if _clients is None:
        try:
            logger.info("Initializing Telegram clients (lazy mode)...")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Clients are initialized lazily when first needed
...
async def dl_file(request: Request):
    try:
        ...
        channel = getattr(file, 'source_channel', None) if getattr(file, 'is_fast_import', False) else STORAGE_CHANNEL
...
async def api_get_directory(request: Request):
    ...
    if data["path"] == "/trash":
        ...
        folder_data = convert_class_to_dict({"contents": contents}, isObject=False, showtrash=True, sort_by=sort_by, sort_order=sort_order)

    elif "/search_" in data["path"]:
        query = urllib.parse.unquote(data["path"].split("_", 1)[1])
        contents = DRIVE_DATA.search_file_folder(query)
        folder_data = convert_class_to_dict({"contents": contents}, isObject=False, showtrash=False, sort_by=sort_by, sort_order=sort_order)
...
async def upload_file(
    ...
    try:
        ...
        logger.error(f"Upload error for id {id}: {e}")
        file_location.unlink(missing_ok=True)
        SAVE_PROGRESS[id] = ("failed", file_size, total_size)
        raise


# Progress, cancel, rename, trash, delete, move, copy, folder tree, etc.
# (Copy-paste your original logic for these if you want minor tweaks — they are mostly fine)
...
async def smart_bulk_import(request: Request):
    ...
    from utils.clients import get_client  # Prefer this if available
    ...
    try:
        client = get_client() if hasattr(utils.clients, "get_client") else (await get_clients())
...
# Add any missing endpoints here if needed
